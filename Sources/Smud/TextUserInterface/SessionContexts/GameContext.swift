...
final class GameContext: SessionContext {
    ...
    func greet(session: Session) {
        ...
        session.sendPrompt(prompt, socket: session.socket)
    }
    ...
    func processResponse(args: Scanner, session: Session) throws -> ContextAction {
        ...
        router.process(args: args, creature: creature, session: session,
            unknownCommand: { context in
                context.send("Unknown command: \(context.args.scanWord().unwrapOptional)", socket: session.socket)
            },
            partialMatch: { context in
                context.send("Warning! Part of your input was ignored: \(context.args.scanRestOfString().unwrapOptional)", socket: session.socket)
            }
        )
